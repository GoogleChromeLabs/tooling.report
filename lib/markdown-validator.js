...
const ignoreCache = !!process.env.IGNORE_CACHE;

const validResults = new Set(['pass', 'fail', 'partial']);
export default async function validateMarkdownData(id, data, rollupContext) {
...
async function augmentGitHubIssueData(id, data) {
  ...
  data.issue = await Promise.all(
    data.issue.map(async issue => {
      if (!ignoreCache && (await ghCache.has(issue))) {
        const { value } = await ghCache.get(issue);
      ...
      await ghCache.set(issue, JSON.stringify(result));
      return result;
    }),
  );
}
