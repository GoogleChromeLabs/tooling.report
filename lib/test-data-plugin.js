...
async function readTest(pathArr) {
      ...
      testSubjects
        .filter(subject => {
          ...
          if (!hasResult && !hasSubtests) {
            console.warn(yellow(`Missing ${subject} test for ${importPath}`));
          }
          return hasResult;
        })
        .map(subject => [
  ...
  if (hasSubtests) {
    test.subTests = await readTests([...pathArr, subtestFolderName]);
  }
 ...
 * @param {string[]} pathArr Path to read, as an array of strings.
 */
async function readTests(pathArr) {
  const entries = await fs.readdir(joinPath(...pathArr), {
    withFileTypes: true,
  });
  ...
  return Object.fromEntries(
    await Promise.all(
      dirs.map(async dir => [dir, await readTest([...pathArr, dir])]),
    ),
  );
}
...
export default function testDataPlugin() {
  return {
    ...
    async load(id) {
      ...
      const obj = await readTests(['tests']);
